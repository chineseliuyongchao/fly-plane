...
public class SoliderShoot : MonoBehaviour 
{
    private Light light;
    public float attack = 50;//伤害
    ...
    void Shoot()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo))//碰撞检测
        {
            ...
            if (hitInfo.collider.tag == Tags.airplane)//判断当前有没有碰撞到敌人
            {
                hitInfo.collider.GetComponent<PlaneHealth>().TakeDamage(attack);
            }
        }
        else
        {
